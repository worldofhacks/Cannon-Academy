...
| T-002 | Safe arithmetic expression and constraint predicate evaluator (no eval) | in-progress | — | `ticket/T-002-safe-expr-eval` | capable | — |
...
| id | title | status | deps | branch | model | issue |
|----|-------|--------|------|--------|-------|-------|
| T-005 | Distractor construction and plausibility validation | backlog | T-002, T-003, T-004 | `ticket/T-005-distractors` | capable | — |
...
- **`T-004`'s ACs deliberately pin bounds, ordering, and monotonicity rather than values** for
  ...
  by the human first. Two of its bounds are *derived, not chosen* and must not be loosened:
...
| F2 | T-013 AC-11 restated over *required* fields; T-013 and T-020 now both require literal, non-derived phase/event enumerations in their tests. |
