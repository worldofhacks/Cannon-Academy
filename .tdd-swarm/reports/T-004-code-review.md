...
| Every export exists, is `as const` / frozen, has an explicit type                                         | **met, one nit**  | All 32 required exports present. The four object exports carry explicit `Readonly<Record<…>>` annotations and are deep-frozen. The 28 scalars have no type annotation and rely on `const` literal-type inference — which is the correct choice (annotating `: number` would _widen_ and discard the literal type), but see **M-5** for the one downstream consequence. |
...
relationship and it holds. It also means T-008 AC-13 (which requires _every one_ of 1,000 seeded
...
volleys; and _any_ answer inside the Perfect-Shot window — the common case in a guided tutorial
...
_don't_ sink in two — is not in the criterion. The frozen tests reached for it
...
ladder (a cannon is earned at _its own_ island's range, so on arrival at island N the best gun in
...
reasonable _without_ `long_nine` — on arrival with `powder_keg` it is a 5-volley duel; once
...
The one outlier is not a hull problem: `double_broadside` (16–28) is unlocked at _Port Sumwich
tier 2_ but is a grade 2–3 gun, so a placement-graded player carrying it flattens
...
| `DISTRACTOR_MAX_RATIO` × T-005's 9-rung ladder                            | rung 7 (`x·2`) is plausible only when `RATIO ≥ 2`. `2` is exactly the boundary; AC-5's `> 1` would have allowed `1.5` and silently killed the rung while `MAX_DISTRACTOR_ATTEMPTS = 9` still claimed 9 reachable rungs | **M-3 — Minor** (value is right; the _bound_ was loose) |
...
a bot at 50 %: _more_ accurate than the child.
...
cohort mercy exists to protect.** The residual critique — that the _tracking_ mechanic is inert
...
A grade-2/3 player carrying `double_broadside` (16–28, unlocked at _Port Sumwich_ tier 2) sinks
...
No AC covers this — AC-3 and T-008 AC-16 both measure the _endpoints_ (`elapsed = 0` vs
...
_bound_ is the loose one. **Recommend** noting on the dev screen that `DISTRACTOR_MAX_RATIO`
...
The ticket claims to define "the _complete_ constant surface up front, so no later ticket has to
...
`export const PERFECT_SHOT_BONUS_DAMAGE = 1;` has type `1`, not `number`. This is the _right_
...
1. `tuning.ts:35` — `PLAN.md §The duel loop: "enemy sloops 40–50 vs the player's 100"`. That
   ...
   quote is an ARCHITECTURE phrasing with `starter` → `enemy`, attributed to PLAN. The _fact_
