...
The implementer's framing is correct and worth preserving: a guard on the two _input boundaries_
sits on either side of the offending arithmetic node, whereas a guard on the _product of every node_
...
| `min(a * a, 1)` — where the _result_ would have been finite | `NON_FINITE_VALUE` |
...
## Is the environment case correctly _runtime_ rather than static?
...
text, so it is rejected statically in `tokenize:155` and is _not_ suppressible — verified:
...
Height increases by exactly 1, only at those three sites, each checked. So _every node in a
completed tree has height ≤ 1024_ holds by induction — the guarantee is structural, not
probabilistic.
...
A _mixed_ chain (`1+1*1-1/1…`) accepts **1,535** terms. That is correct, not a leak: `*` and `/`
bind tighter, so the product sub-chains nest _inside_ the sum chain and tree height stays ≤ 1024.
...
I lifted `MAX_AST_DEPTH` in a scratch copy of the _shipped_ module (teeth-checked per L-014: an
...
1. **The docblock's numbers are Node's, presented as host-general.** `:230-239` cites "a first
   ...
   margin is 3.4×, and at 0.5 MB it is 1.5×. The claim is not wrong about the _direction_, but it
...
threshold the bisection's next-larger case sometimes succeeded on retry. That is an argument _for_
...
200- and 500-term chains — **all evaluate, all in 0–1 ms.** In particular there is no _slow_ `gcd`
...
1. **[carried over] The static/dynamic split still leaks for an own key holding `undefined`.**
   ...
   short-circuit. Mitigating: AC-25 now classifies environment _values_ as a runtime concern, which
