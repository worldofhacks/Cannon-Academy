...
The five newest criteria (AC-16…AC-20) are, if anything, the *best* covered in the file. AC-16's
...
The `@ts-expect-error` at `:970` correctly *fails* (TS2578 "unused directive") the moment
...
*position*, or a recovery strategy. Recursive-descent, Pratt/precedence-climbing, and a
...
validate-then-emit design). Error *messages* are constrained only where the ticket constrains
...
authoritative guard is ESLint, which matches identifier *references* rather than text." I probed
...
**3 of 8 caught.** `no-eval` matches the *binding named* `eval`, and `no-new-func` matches the
*binding named* `Function`; neither follows an alias, a computed member access, or reflection. The
...
Poison every route to dynamic compilation *before* the module is imported, so a reference cached
...
describe('AC-1 — the evaluator constructs no code, at import time or at call time', () => {
  ...
  beforeEach(() => {
    tripped = [];
    const trap = (what: string) => (...args: unknown[]) => { tripped.push(what); void args; return () => 0; };
    ...
    Object.defineProperty(realFn.prototype, 'constructor', {
      configurable: true,
      get() { tripped.push('Function.prototype.constructor'); return trap('fn.constructor'); },
    });
  });
...
those six work and rejects five *non-function* names — `foo`, `constructor`, `Math`, `ABS`, `a`.
...
criterion listing only *accept* cases is satisfied by an evaluator that accepts everything.
...
## M1. Out-of-grammar *numeric literal* forms are untested
...
`require('fs')`, `globalThis['eval']` — but every numeric entry is about the *decimal point*.
...
`foo(z)` (unknown function *and* unknown identifier), `min(z)` (arity *and* unknown identifier),
and `z / 0` (unknown identifier *and* division by zero) each have two defensible codes and no
