...
/**
 * The captain's Uncharted Sea progress (A-079, amended D-17).
 *
 * This envelope is the ONLY place a generated island may live on the captain. The bus law
 * (`docs/ENDLESS-ARCHIPELAGO-DESIGN.md` §1) is what this field exists to protect: `currentIsland`
 * and `unlockedIslands` carry authored ids or null, forever — during Uncharted play the authored
 * bus stays parked at the last authored island, and gen progress moves HERE instead. The docs are
 * whole `GenIslandDoc`s rather than ids because a generated island has no catalog to look itself
 * up in; both slots are regenerable from `(seed, index, band)`, which is why persistence may
 * reset a corrupt slot to null without losing anything.
 */
export interface UnchartedState {
  /** Frontier islands cleared. Never negative; persistence clamps it on hydrate. */
  clearedCount: number;
  /** The island the captain is sailing now, or null before the frontier is dealt. */
  current: GenIslandDoc | null;
  /** The island already generated beyond `current`, so arrival never waits on generation. */
  next: GenIslandDoc | null;
  /** First-arrival host latch for the Uncharted Sea — lives here, never in `seenEncounters`. */
  metLumen: boolean;
}

/** Fresh Uncharted envelope — a FUNCTION for the same aliasing reason as `freshMercyState`. */
export function freshUnchartedState(): UnchartedState {
  return { clearedCount: 0, current: null, next: null, metLumen: false };
}
...
export interface Captain {
  ...
  /**
   * Uncharted Sea progress (A-079, amended D-17) — the one captain field that may hold generated
   * islands. Optional and tolerated-as-absent, the `seenEncounters`/`metRivals` ruling exactly:
   * defaulted in `emptyCaptain`, normalized in both of `persistence.ts`'s arms, deliberately NOT
   * part of `isBaseCaptain`, and NOT a reason to bump `SCHEMA_VERSION` — a bump without a
   * migration arm deletes every live save (see the note on `onboardingBeat`).
   */
  uncharted?: UnchartedState;
}
...
export interface CaptainState {
  ...
  /**
   * The Uncharted Sea actions (A-079, work item 4) — pure state moves, no service logic. The
   * docs they store are generated by `src/services/uncharted/generator.ts` and validated by the
   * caller (A-081's orchestration); the store's job is only to hold them. None of these may
   * touch `currentIsland` or `unlockedIslands` — the bus law.
   */
  /** Ensures the envelope exists. Idempotent: never resets progress an existing envelope holds. */
  beginUncharted: () => void;
  /** Deals (or re-deals) the frontier pair. Slots are regenerable from seed, so null is legal. */
  setUnchartedIslands: (current: GenIslandDoc | null, next: GenIslandDoc | null) => void;
  /** A frontier win: clearedCount+1, `next` promotes to `current`, the caller supplies the new next. */
  advanceUnchartedState: (next: GenIslandDoc | null) => void;
  /** Latches the Uncharted host as met. A latch: written true once, never back to false. */
  markLumenMet: () => void;
...
export function createCaptainStore(initial?: Captain): CaptainStore {
  return createStore<CaptainState>((set, get) => ({
    ...
    // Idempotent by returning the SAME state when the envelope exists: `set` sees an identical
    // reference and does nothing, so re-entering the Uncharted Sea can never reset a cleared
    // count, a dealt frontier, or the Lumen latch. The envelope is normally always present
    // (emptyCaptain and both persistence arms default it); this is the guard that makes the
    // optional field safe to read after `beginUncharted` regardless of how a captain was built.
    beginUncharted: () =>
      set((s) =>
        s.captain.uncharted === undefined
          ? { captain: { ...s.captain, uncharted: freshUnchartedState() } }
          : s,
      ),

    setUnchartedIslands: (current, next) =>
      set((s) => ({
        captain: {
          ...s.captain,
          uncharted: { ...(s.captain.uncharted ?? freshUnchartedState()), current, next },
        },
      })),

    // A pure move, not a settlement side effect (design §2 S4): the cleared island is the old
    // `current`, `next` slides forward, and the CALLER deals the new next — the store never
    // generates. `metLumen` and everything outside the envelope ride through untouched.
    advanceUnchartedState: (next) =>
      set((s) => {
        const uncharted = s.captain.uncharted ?? freshUnchartedState();
        return {
          captain: {
            ...s.captain,
            uncharted: {
              ...uncharted,
              clearedCount: uncharted.clearedCount + 1,
              current: uncharted.next,
              next,
            },
          },
        };
    ...
    markLumenMet: () =>
      set((s) => ({
        captain: {
          ...s.captain,
          uncharted: { ...(s.captain.uncharted ?? freshUnchartedState()), metLumen: true },
        },
      })),
