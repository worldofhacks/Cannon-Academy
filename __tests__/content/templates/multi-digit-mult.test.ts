/**
 * A-069 / D-14 — `multi_digit_mult` golden suite (4.NBT.5): 2–3-digit × 1-digit, and
 * tens × tens.
 *
 * D-14 (`tickets/app/OWNER-RULINGS.md`) adds this skill as Product Peaks' G4-5 curriculum —
 * `islands.json` `isla_products.curriculum.g4_5` — paid by the `carronade`. This suite is the
 * content contract for `src/content/templates/multi_digit_mult.json`: schema validity, id
 * hygiene, the A-069 AC-3 golden sweep (30 seeds, zero CONSTRAINTS_UNSATISFIED, zero
 * DISTRACTOR_FAILURE, ladder fill < 25%), the × glyph rule (never `*`, never a lowercase `x`
 * operator — T-015's mult_facts convention), one-digit-factor bounds on the ticketed shapes,
 * word-problem readability, and hand-verified seed-pinned literals for frozen seed-replay.
 *
 * The `{a}0` / `{a}00` text tokens compose the tens/hundreds numerals at render time, the same
 * trick `place_value_teens` uses for `1{a}` — the sampled param is the leading digit.
 */
...
const SKILL = 'multi_digit_mult' as const;
const FILE = 'multi_digit_mult.json';

/** 999 × 9 = 8,991 is the largest ticketed product; nothing rendered or answered exceeds it. */
const DISPLAY_BOUND = 8991;

const EXPECTED_IDS = [
  'multi_digit_mult_two_by_one',
  'multi_digit_mult_three_by_one',
  'multi_digit_mult_tens_by_tens',
  'multi_digit_mult_product_first',
  'multi_digit_mult_missing_factor',
  'multi_digit_mult_hundreds_by_one',
  'multi_digit_mult_word_crates',
  'multi_digit_mult_times_ten',
] as const;

/**
 * Hand-verified arithmetic at pinned seeds (frozen seed-replay): 12 × 6 = 72, 104 × 6 = 624,
 * 20 × 60 = 1200, 864 ÷ 9 = 96 (the missing factor), 476 ÷ 7 = 68.
 */
const SPOT_CHECKS: readonly SpotCheck[] = [
  { id: 'multi_digit_mult_two_by_one', seed: 1, text: '12 × 6 = ?', answer: 72 },
  { id: 'multi_digit_mult_two_by_one', seed: 7, text: '17 × 9 = ?', answer: 153 },
  { id: 'multi_digit_mult_three_by_one', seed: 1, text: '104 × 6 = ?', answer: 624 },
  { id: 'multi_digit_mult_three_by_one', seed: 7, text: '157 × 9 = ?', answer: 1413 },
  { id: 'multi_digit_mult_tens_by_tens', seed: 1, text: '20 × 60 = ?', answer: 1200 },
  { id: 'multi_digit_mult_product_first', seed: 1, text: '? = 12 × 6', answer: 72 },
  { id: 'multi_digit_mult_missing_factor', seed: 1, text: '9 × ? = 864', answer: 96 },
  { id: 'multi_digit_mult_missing_factor', seed: 7, text: '7 × ? = 476', answer: 68 },
  { id: 'multi_digit_mult_hundreds_by_one', seed: 1, text: '100 × 6 = ?', answer: 600 },
  {
    id: 'multi_digit_mult_word_crates',
    seed: 1,
    text: 'A ship hauls 12 crates with 6 cannonballs in each crate. How many cannonballs are in the hold?',
    answer: 72,
  },
  { id: 'multi_digit_mult_times_ten', seed: 1, text: '11 × 10 = ?', answer: 110 },
];
...
describe('A-069 — multi_digit_mult authoring contract', { timeout: 60000 }, () => {
  ...
  it('spec(A-069:AC-3) multiplication renders as ×: never *, never a lowercase x operator', () => {
    for (const template of templates()) {
      if (template.isWordProblem !== true) {
        expect(template.text, template.id).toContain('×');
      }
      expect(template.text, template.id).not.toContain('*');
      expect(usesLowercaseXAsOperator(template.text), template.id).toBe(false);
    }
  });
});

describe('A-069 — multi_digit_mult golden sweep', { timeout: 60000 }, () => {
  ...
  it('spec(A-069:AC-3) every single-digit factor stays one digit across the sweep (4.NBT.5 shapes)', () => {
    // The ticketed shapes multiply BY one digit (or by 10, or tens × tens); the sampled
    // one-digit factor params are pinned by their declared ranges, re-checked here.
    for (const template of templates()) {
      for (const [name, range] of Object.entries(template.params)) {
        if (name !== 'b') continue;
        expect(range[0], `${template.id}.${name}`).toBeGreaterThanOrEqual(2);
        expect(range[1], `${template.id}.${name}`).toBeLessThanOrEqual(9);
      }
    }
  });

  it('spec(A-069:AC-3) answers and text numerals are non-negative integers within the product bound', () => {
    for (const template of templates()) {
      for (let seed = 1; seed <= SWEEP_SEEDS; seed += 1) {
        const question = generateOne(template, seed);
  ...
  it('spec(A-069:AC-3) word problems stay under 140 chars, end with ?, and carry a numeral', () => {
    for (const template of templates()) {
